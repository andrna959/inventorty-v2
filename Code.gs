/**
 * ==========================================================
 * CODE.GS - VERSI FINAL (NOTES + DATE FIX + STABIL)
 * ==========================================================
 */
const CONFIG = {
...
function getBatchStockList(filterType) {
  ...
  // Gunakan getDisplayValues agar aman dari error tanggal
  ...
  for (let i = 0; i < data.length; i++) {
    ...
    let item = {
      idBatch: r[0], kode: r[1], nama: r[2], brand: r[3], 
      qty: Number(r[4]) || 0,
      typeLoc: r[5], lokasi: r[6], batch: r[7],
      exp: r[8], sticker: r[9], cover: r[10], size: r[11], lastUpdate: r[12]
    };
...
// --- 4. BACA HISTORY (MONITORING) - UPDATE: AMBIL NOTES ---
function getInboundHistory(start, end) {
  ...
  // Ambil 17 Kolom sebagai TEXT (A-Q)
  const data = sheet.getRange(2,1,lastRow-1, 17).getDisplayValues(); 
  ...
  for (let i = 0; i < data.length; i++) {
    let r = data[i];
    if (!r[0]) continue; // Skip jika ID kosong

    let rawDate = String(r[1]).trim(); // Tanggal Teks Asli
    let dateForFilter = rawDate; 

    // Normalisasi Tanggal DD/MM/YYYY -> YYYY-MM-DD untuk Filter
    ...
    let pass = true;
    if (isFilterOn) {
        // Cek hanya jika format valid YYYY-MM-DD
        if (dateForFilter.match(/^\d{4}-\d{2}-\d{2}$/)) {
            if (dateForFilter < start || dateForFilter > end) pass = false;
        } else {
            pass = true; // Jika format aneh, loloskan saja (Fail Safe)
        }
    }
    ...
    if (pass) {
      result.push({
        ...
        typeLoc: r[14], 
        lokasi: r[15],
        notes: r[16] || "-" // AMBIL NOTES (Kolom Q / Index 16)
      });
    }
  }
...
function addSupportItem(cat, v1, v2) {
  ...
  s.getRange(r, col).setValue(v1);
  if(v2) s.getRange(r, col+1).setValue(v2);
  return {success:true};
}
...
function deleteSupportItem(cat, val) {
  ...
  for(let i=0; i<d.length; i++) {
    if(String(d[i][col-1]) == String(search)) { s.getRange(i+1, col).clearContent(); if(cat=='lokasi') s.getRange(i+1, col+1).clearContent(); break; }
  }
}
function addMasterData(form) {
  const s = _ensureSheet("MasterProduk");
  s.appendRow([getNextId(), form.nama, form.brand, form.notes]);
  return {success:true};
}
function updateMasterData(form) {
  ...
  for(let i=1; i<d.length; i++) {
    if(String(d[i][0]) == form.kode) { s.getRange(i+1, 2).setValue(form.nama); s.getRange(i+1, 3).setValue(form.brand); s.getRange(i+1, 4).setValue(form.notes); return {success:true}; }
  }
}
function deleteMasterData(kode) {
  ...
  const d = s.getDataRange().getValues();
  for(let i=1; i<d.length; i++) {
    if(String(d[i][0]) == kode) { s.deleteRow(i+1); return {success:true}; }
  }
}
function getNextId() {
  const s = _ensureSheet("MasterProduk");
  if(s.getLastRow()<2) return "SNB-0001";
  const last = s.getRange(s.getLastRow(), 1).getValue();
  const num = parseInt(last.split("-")[1]) + 1;
  return "SNB-" + String(num).padStart(4,'0');
}
function deleteInboundTransaction(id) {
  const s = _ensureSheet("StokMasuk");
  const d = s.getDataRange().getValues();
  for(let i=1; i<d.length; i++) {
    if(String(d[i][0]) == id) { s.deleteRow(i+1); return {success:true, message:"Dihapus"}; }
  }
